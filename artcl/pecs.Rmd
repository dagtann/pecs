...
pdta <- bind_rows(tmp, tmp2) %>%
    ...
    mutate(
        key_labels = factor(
            ...
            labels = c("Turnout", "PEC contested", "PEC votes")
        ),
...
few if  New Zealands move from a
majoritarian to a proportional system
...
, though Heralds of pre-electoral coaltions should not immediately abondon ship, because


 This is no foregone conclusion though,


The principal objection to the exploratory analysis above

Figure \ref{fig:edaCommonTrend} makes the same point from a different
perspective.
...
```{r, reportDetrendedResults, eval=FALSE}
rm(list = ls()[!(ls() %in% clean_workspace)])
...
treatments <- c("pec1", "vote_pec")
response <- "turnout"
trends <- c("None" = "None", "Twoways" = "twoways", "FD" = "fd",
    "Common trend" = "t", "Unit-specific Trend" = "t * iso3c","LDV" = "turnout_l1"
)

# Prepare data
data_to_fit <- filter(country_panel, in_tillman == 1) %>% filter((iso3c %in% c("LUX", "CAN")) == FALSE)
data_to_fit <- group_by(data_to_fit, iso3c) %>%
...
for (trend in trends){
    for(treatment in treatments) {
        ...
        if (trend == "None") {
            assign(
                "frm", paste0(response, " ~ ", paste(treatment, control, sep = " + "))
            )
        }
        if (trend == c("twoways")) {
            assign("frm", paste0(response, " ~ ", paste(treatment, control, sep = " + ")))
        ...
        if (trend == c("fd")) {
            assign("frm", paste0(response, " ~ ", paste(treatment, control, sep = " + ")))
            fitted_models[[paste(trend, treatment, sep = "_")]] <- plm(
                as.formula(frm), effect = "individual", model = "fd",
                data = data_to_fit
            )
            next
        }
...
bk_se <- lapply(fitted_models, function(x) vcovBK(x, cluster = "group"))
bk_p <- lapply(fitted_models, function(x){
        lmtest::coeftest(x, vcov = vcovBK(x, cluster = "group"))[, 4]
    }
)

vcovBK(fitted_models[[-5]], cluster = "time")

texreg(
    fitted_models, file = file.path(path_project, "artcl", "detrended_models.tex"),
    digits = 2,
    omit.coef = "(t:)|(factor\\(year\\))",
    override.se = lapply(bk_se, function(x){sqrt(diag(x))}),
    override.p = lapply(fitted_models, function(x){
        lmtest::coeftest(x, vcov = vcovBK(x, cluster = "group"))[, 4]
    }),
    custom.coef.names = c("PEC", "Parties", "Disproportionality", "PR",
        ...
        "Vote PEC", "$t$", "Turnout$_{t-1}$"
    ),
    reorder.coef = c(1, 11, 2:10, 12:13),
    booktabs = TRUE, sideways = TRUE,
    stars = c(0.001,  0.01, 0.05, 0.1),
    custom.note = "Panel corrected SEs in parantheses \\cite{Beck.1995}, clustered by country. Country-specific growth curves. %stars."
)
# Clean up
...
tmp[grepl("[a-z]\\_[a-z]", tmp)] <- paste("Technique", "\\multicolumn{2}{c}{None}",
    "\\multicolumn{2}{c}{Year FE}", "\\multicolumn{2}{c}{Linear Trend}",
    "\\multicolumn{2}{c}{Unit-specific Trend}", "\\multicolumn{2}{c}{LDV} \\\\",
    sep = " & "
