...
response <- "turnout"
trends <- c("factor(year)", "t_lin", "t_lin + t_squ + t_cub", "t_lin * iso3c")

# Prepare data
t_polynomials <- poly(tillman[["year"]], 3)
colnames(t_polynomials) <- c("t_lin", "t_squ", "t_cub")
data_to_fit <- cbind.data.frame(tillman, t_polynomials)
...
texreg(fitted_models, file = file.path(path_project, "artcl", "detrended_models.tex"),
    digits = 2,
