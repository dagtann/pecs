...
```{r, edaCommonTrend, fig.cap="Secular Trends in the Data"}
rm(list = ls()[!(ls() %in% clean_workspace)])
...
response <- "turnout"
trends <- c("None", "factor(year)", "t_lin", "t_lin + t_squ + t_cub", "t_lin * iso3c")

# Prepare data
data_to_fit <- filter(country_panel, in_tillman == 1)
t_polynomials <- poly(data_to_fit[["year"]] - median(data_to_fit[["year"]]), 3)
colnames(t_polynomials) <- c("t_lin", "t_squ", "t_cub")
data_to_fit <- cbind.data.frame(data_to_fit, t_polynomials)
...
texreg(
    ...
    reorder.coef = c(1, 11, 2:10, 12:14),
...
) # NOTE: header changed manually in file
# housekeeping
for (p in packages) detach(paste("package", p, sep = ":"), character.only = TRUE)
rm(list = ls()[!(ls() %in% clean_workspace)])
```

\include{detrended_models_manual_header}
