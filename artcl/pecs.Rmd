...
across \textit{all} measurement occasions \cite[482]{Wooldridge.2009}. Despite their appeal, fixed-effects designs have recently come under attack [add
...
```{r, edaNonStationarity, fig.cap="Visual Inspection of Non-Stationarity"}
...
for each of Tillman's pivotal variables.\footnote{
    ...
    the electoral strength of pre-electoral coalitions. For example, one binary indicator takes the value of 1 whenever at least one pre-electoral
...
\begin{enumerate}
    \item As documented in extant research average turnout has been declining
        ...
        the observation period in 2011. Pre-electoral coalitions exhibit a more complex, though not  entirely unrelated trend. As shown in the middling panel of the top row, PECs become more prevalent until the mid-1980s. Later on, their attraction nosedives and continously declines -- in
    ...
    \item Turning to their second moment, evidence against stationarity of the
        data is not not as clear-cut. On the one hand, the standard deviation of about the annual average in turnout has monotonically increased over
    ...
    \item Finally, Figure \ref{fig:edaCommonTrend} in the Appendix
        disaggregates trends in turnout and pre-electoral coalitions to the country-level. As can be seen from the individual panels, all
...
diagnostics, which all evaluate Tillman's pivotal regression models \cite[733, Table 4; Models 2 and 4]{Tillman.2015}. The Durbin–Watson statistic detects
first-order autocorrelation among the residuals. Its classical form does not
account for panel-data and has therefore been generalized by
...
\begin{table}
    ...
    \begin{tabular}{lcc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Tests for Serial Correlation in the Residuals}
    \label{tbl:TestsSerialCorrelation}
\end{table}


What to do with non-stationary panel data? One solution is to somehow include
the trend component explicitly into the regression model. The options are many.
A two-way error components model is akin a unit-fixed-effects model that also
includes time dummies. Both eliminate unobserved heterogeneity among the
\textit{time points}. Alternatively, a time index component can be included as
an independent variable to reduce any confounding due to trends in the data. A
somewhat more stringent version of this approach interacts the time index with
the unit-fixed effects and thereby estimates unit-specific trends. A second
remedy against non-stationarity is to transform the data, e.g., by taking
first-differences or by applying a Prais-Winston transformation\footnote{
    We did not get around to do the Prais-Winston regression yet.
}.
Finally, dynamic-panel models have been suggested as means to deal with
serial correlation in panel data [REFERENCE]. In political science, the perhaps
best known variant of this approach goes back to \cite{Beck.1995} and turns on
a one-period lag of the dependent variable along with panel-corrected standard
...
Results for each\footnote{
    Again, we didn't get around to do Prais-Winston, yet.
}
of the previously described variations on the fixed-effects scheme are shown in
Figure \ref{fig:reportDetrendedResults}. In the top-row it plots the marginal
effect of PEC formation on turnout. In the bottom row it shows the same
information for the total vote share of PECs.\footnote{
    Following the original publication, turnout and Vote PEC are measured in
    percent.
}
Confidence intervals at the 90 and 95 percent levels have been added to each plot.\footnote{
    All models use panel-corrected standard errors clustered by country.
}
The left-most column replicates Tillman's original finding: Pre-electoral
coalitions boost turnout (top) and this effect increases in their vote share
(bottom). Each confidence interval excludes $0$, and therefore conventional
standards of statistical significance are met. Things change markedly
once non-stationarity is taken into account. On the one hand, the mobilizing
potential of PEC entering elections is notably reduced and its confidence intervals consistently either include $0$ or exclude it by a narrow margin.
...
once
 ...
 once
turn immediately gloomy as we move towards the right. Note how the binary
the binary indi
...
```{r, reportDetrendedResults, fig.cap="Detrended Effect Estimates"}
rm(list = ls()[!(ls() %in% clean_workspace)])
...
ggplot(data = pdta, aes(x = trend_label, y = beta)) +
    ...
    labs(x = "Correction for Non-Stationarity", y = expression(hat(beta)),
...
## Exploratory Analysis: Which Pre-electoral Coalitions matter?

```{r, edaRelFreqVariantsPEC}
rm(list = ls()[!(ls() %in% clean_workspace)])
alpha <- .05
...
mu <- aggregate(value ~ key, data = pdta, FUN = mean, na.rm = TRUE)
names(mu) <- c("key", "mu")
sigma <- aggregate(value ~ key, data = pdta, FUN = function(x) {
...
names(sigma) <- c("key", "sigma")
pdta <- inner_join(mu, sigma, by = "key")
pdta <- mutate(pdta,
    lower95 = mu - qnorm(alpha / 2, lower = FALSE) * sigma,
    upper95 = mu + qnorm(alpha / 2, lower = FALSE) * sigma,
    shape_label = factor(
        ifelse(as.character(key) %in% paste0("any_type", seq(7) - 1),
            "PEC Type", "Other"
        ),
        levels = c("PEC Type", "Other")
    ),
    key_label = factor(key,
        levels = c(paste0("any_type", seq(7) - 1), "any_prog", "any_incumbent"),
        labels = c("Don't know", "Nomination\nAgreement", "Joint list",
...
ggplot(
    data = pdta, aes(x = key_label, y = mu, ymin = lower95, ymax = upper95, shape = shape_label)
) +
    geom_linerange() + geom_point(fill = "white") + scale_shape_manual(values = c(21, 23)) +
    labs(
        y = "Relative Frequency", shape = "",
        caption = "Note: CIs at 95 percent added."
    ) +
    ggthemes::theme_fivethirtyeight() +
