...
# constants
pec_indicators <- paste0("pec", seq(7))
...
sum(!is.na(tmp[, -1])) <= sum(country_panel$nupec_neu, na.rm = TRUE)
# sanity check: n is.na(type) + n !(is.na(type)) == nupec_neu
sum(apply(tmp[, -1], 1, flag_nas)) + sum(!is.na(tmp[, -1])) == sum(country_panel$nupec_neu, na.rm = TRUE)
